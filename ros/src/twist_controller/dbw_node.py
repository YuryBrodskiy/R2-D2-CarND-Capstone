...
You will subscribe to `/twist_cmd` message which provides the
proposed linear and angular velocities.
You can subscribe to any other message that you find important or
refer to the document for list of messages subscribed to by the
...
While in the simulator, its enabled all the time, in the real car, that will
not be the case. This may cause your PID controller to accumulate
error because the car could temporarily be driven by a human
instead of your controller.

We have provided two launch files with this node. Vehicle
specific values (like vehicle_mass, wheel_base) etc should not
be altered in these files.

We have also provided some reference implementations for PID
controller and other utility classes. You are free to use them
or build your own.

Once you have the proposed throttle, brake, and steer values,
publish it on the various publishers that we have created in
...
class DBWNode(object):
    def __init__(self):
        ...
        self.velocity_sub = rospy.Subscriber(
            '/current_velocity', TwistStamped, self.velocity_cb, queue_size=1)
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            # `twist_controller`. You should only publish the control
            # commands if dbw is enabled.
            if self.velocity is not None and \
               self.twist_cmd is not None:
                toc = rospy.get_time()
                delta_t = toc - self.tic
                self.tic = toc
                throttle, brake, steer = self.controller.control(
                    self.twist_cmd.twist.linear.x,  # proposed linear velocity
                    self.twist_cmd.twist.angular.z,  # proposed angular velocity  # noqa
                    self.velocity.twist.linear.x,  # current linear velocity
                    self.dbw_enabled,  # dbw status
                    delta_t)  # any other argument you need
                if self.dbw_enabled:
                    self.publish(throttle, brake, steer)
            rate.sleep()

    def publish(self, throttle, brake, steer):
        rospy.logwarn('DBW: Publish')
        tcmd = ThrottleCmd()
        tcmd.enable = True
