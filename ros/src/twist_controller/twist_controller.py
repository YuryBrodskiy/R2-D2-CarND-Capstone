...
class Controller(object):
    def __init__(self, ps):
        ...
        self.ps = ps
        self.driverless_mass = self.ps.vehicle_mass + \
            self.ps.fuel_capacity * GAS_DENSITY
        self.pid_steer = PID(
            .5, .01, .25, -ps.max_steer_angle, ps.max_steer_angle)
    ...
    def control(self, twist_cmd, c_v, t_delta, pose, waypoints):
        ...
        if accel > 0:
            throttle = torque / (self.driverless_mass *
                                 self.ps.accel_limit * self.ps.wheel_radius)
            throttle = min(1.0, throttle)
        else:
            brake = self.driverless_mass * abs(self.ps.decel_limit) * \
                self.ps.wheel_radius
            brake = min(abs(torque), brake)

        # Zero-out for deadband
        if abs(accel) < self.ps.brake_deadband:
            rospy.logwarn("Controller: Running in deadband!")
            throttle, brake = 0, 0
